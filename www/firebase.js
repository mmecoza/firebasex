...
exports.writeBatchToFirestoreCollection = function (document, collection, timestamp, success, error) {
    if(typeof collection !== 'string') return error("'collection' must be a string specifying the Firestore collection name");
    if(document.constructor.name !== 'Array' && typeof document.length === 'number') return error("'document' must be an array of objects specifying record data");
    ...
    exec(success, error, "FirebasePlugin", "writeBatchToFirestoreCollection", [document, collection, timestamp || false]);
};
...
exports.getFileUrlFromStorage = function (guri, success,error) {
    if (typeof guri !== 'string') return error(`'Uri' must be a string specifing google bucket path`);
    exec(success, error, "FirebasePlugin", "getFileUrlFromStorage" ,[guri]);
};
